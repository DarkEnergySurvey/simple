...
def computeCharDensity(nside, data, ra_select, dec_select, fracdet=None):
    # 1. Compute the characteristic density
...
def computeLocalCharDensity(nside, data, ra_select, dec_select, x_peak, y_peak, fracdet=None):
    # 2. Compute the local characteristic density
...
def fitAperture(proj, distance_modulus, characteristic_density_local, x_peak, y_peak, angsep_peak, ra_peak_array, dec_peak_array, r_peak_array, sig_peak_array, distance_modulus_array):
    ...
    #print('Candidate: ra: {:12.3f}, dec: {:12.3f}, r: {:12.3f}, sig: {:12.3f}').format(ra_peak, dec_peak, r_peak, np.max(sig_array))
...
def searchByDistance(nside, data, distance_modulus, pix_nside_select, ra_select, dec_select, magnitude_threshold=mag_max, fracdet=None):
    ...
    for index in range(1, n_region + 1): # loop over peaks
        ...
        characteristic_density_local = computeLocalCharDensity(nside, data, ra_select, dec_select, x_peak, y_peak, fracdet)
    ...
    #return angsep_peak, x_peak, y_peak, ra_peak_array, dec_peak_array, r_peak_array, sig_peak_array, distance_modulus_array
    return ra_peak_array, dec_peak_array, r_peak_array, sig_peak_array, distance_modulus_array
...
#    hotspots = fits.read(sim_catalog)
#
#    return hotspots
#
#def loop_over_candidates(hotspots, distance_modulus):
#    ra_peak_array = []
#    dec_peak_array = []
#    r_peak_array = []
#    sig_peak_array = []
...
#        ra_peak = spot['RA']
#        dec_peak = spot['DEC']
#        characteristic_density_local = computeLocalCharDensity() #TODO
#        ra_peak_array, dec_peak_array, r_peak_array, sig_peak_array, distance_modulus_array = fitAperture(distance_modulus, characteristic_density_local, x_peak, y_peak, angsep_peak, ra_peak_array, dec_peak_array, r_peak_array, sig_peak_array, distance_modulus_array)
#
#    return ra_peak_array, dec_peak_array, r_peak_array, sig_peak_array, distance_modulus_array
#
#def search(mode='real'):
#  if mode == 'real':
#    hotspots = find_real_hotspots()
#  elif mode = 'sim':
#    hotspots = find_sim_hotspots()
#  results = loop_over_candidates(hotspots)
